...
class PI_HIQLAgent(flax.struct.PyTreeNode):
    ...
    def distance_grad_s(self, obs, goals, grad_params):
        """Compute the value function gradient with respect to the states"""
        def distance_s1(s, g, params):
            v1, _ = self.network.select('value')(s, g, params=params)
            d1 = -v1
            return d1.mean()
        
        def distance_s2(s, g, params):
            _, v2 = self.network.select('value')(s, g, params=params)
            d2 = -v2
            return d2.mean()
    ...
    def value_loss(self, batch, grad_params):
        ...
        grad_distance_sg1, grad_distance_sg2 = self.distance_grad_s(batch['observations'], batch['value_goals'], grad_params)
        ...
        ratio1 = batch['speed']*grad_norm_d1
        ratio2 = batch['speed']*grad_norm_d2
        eikonal_loss = jnp.mean((ratio1 - 1) ** 2) + jnp.mean((ratio2 - 1) ** 2) 

        if self.config['use_obstacles'] and 'obstacles' in batch and 'speed_obstacles' in batch:
            grad_distance_obs_g1, grad_distance_obs_g2 = self.distance_grad_s(batch['obstacles'], batch['value_goals'], grad_params)

            grad_norm_d_obs1 = jnp.linalg.norm(grad_distance_obs_g1 + 1e-8, axis=-1) 
            grad_norm_d_obs2 = jnp.linalg.norm(grad_distance_obs_g2 + 1e-8, axis=-1) 

            ratio_obs1 = batch['speed_obstacles']*grad_norm_d_obs1
            ratio_obs2 = batch['speed_obstacles']*grad_norm_d_obs2

            eikonal_loss_obs = jnp.mean((ratio_obs1 - 1) ** 2) + jnp.mean((ratio_obs2 - 1) ** 2)

            if self.config['Eikonal_only']:
                value_loss = eikonal_loss + eikonal_loss_obs
                ...
                value_loss = value_loss1 + value_loss2 + eikonal_loss + eikonal_loss_obs

        else:

            if self.config['Eikonal_only']:
                value_loss = eikonal_loss
